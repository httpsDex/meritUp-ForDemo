...
async function downloadSummaryMeritPay() {
    try {
        ...
        const data = await response.json();
        await generateSummaryMeritPayPDF(data);
...
async function generateSummaryMeritPayPDF(data) {
    try {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF('l', 'mm', 'legal'); // Landscape
        
        // First, fetch current pay rates from database
        const payRates = await fetchPayRates();
        ...
        // Helper function to calculate pay based on total points
        const calculatePayForPoints = (points) => {
            if (!payRates || payRates.length === 0) return '0.00';
            
            const totalPoints = parseFloat(points);
            if (isNaN(totalPoints)) return '0.00';
            
            // Find the range that matches the points
            const matchingRange = payRates.find(range => 
                totalPoints >= range.min_points && totalPoints <= range.max_points
            );
            
            return matchingRange ? parseFloat(matchingRange.pay_amount).toFixed(2) : '0.00';
        };
        
        let yPos = 5;
        const leftMargin = 15;
        const pageWidth = 355.6; // Legal landscape width (14 inches = 355.6mm)
        ...
        // ========== TABLE ==========
        // Prepare table data with dynamic pay calculation
        const tableData = data.employees.map((emp, index) => {
            const recommendedPay = calculatePayForPoints(emp.total_points);
            return [
                (index + 1).toString(),
                emp.employee_name,
        ...
        if (data.employees.length > 0) {
            const totals = {
                teaching_competence: data.employees.reduce((sum, emp) => sum + (parseFloat(emp.teaching_competence) || 0), 0),
                effectiveness: data.employees.reduce((sum, emp) => sum + (parseFloat(emp.effectiveness) || 0), 0),
                professional_growth: data.employees.reduce((sum, emp) => sum + (parseFloat(emp.professional_growth) || 0), 0),
                teaching_experience: data.employees.reduce((sum, emp) => sum + (parseFloat(emp.teaching_experience) || 0), 0),
                total_points: data.employees.reduce((sum, emp) => sum + (parseFloat(emp.total_points) || 0), 0),
                total_pay: data.employees.reduce((sum, emp) => {
                    const pay = parseFloat(calculatePayForPoints(emp.total_points)) || 0;
                    return sum + pay;
                }, 0)
            };
            ...
            tableData.push([
                '',
                'TOTAL',
                formatNum(totals.teaching_competence),
                formatNum(totals.effectiveness),
                formatNum(totals.professional_growth),
                formatNum(totals.teaching_experience),
                formatNum(totals.total_points),
                formatNum(totals.total_pay),
                ''
            ]);
        }
        ...
        doc.autoTable({
            ...
            didDrawCell: (data) => {
                ...
                if (data.row.index === tableData.length - 1 && data.row.index > 0) {
        ...
        // ========== MERIT POINTS GUIDE ==========
        // Create guide data from database
        const guideData = payRates.map(range => [
            `${range.min_points} - ${range.max_points}`,
            `Php ${parseFloat(range.pay_amount).toFixed(2)}`
        ]);
        ...
        if (guideData.length > 0) {
            ...
            // Show message if no pay rates configured
            doc.setFontSize(9);
            doc.setFont(undefined, 'italic');
            doc.text('No merit pay ranges configured in the system.', leftMargin, yPos);
            doc.text('Please contact the administrator to set up pay rates.', leftMargin, yPos + 5);
            yPos += 15;
        }
        ...
        // Define signature positions (4 columns for legal size)
...
// Helper function to fetch pay rates
async function fetchPayRates() {
    try {
        const token = localStorage.getItem("accessToken");
        const response = await fetch(`${apiBaseUrl}/api/public/pay-rates`, {
            headers: {
        ...
        if (response.ok) {
            ...
            console.error("Failed to fetch pay rates:", response.status);
            return [];
        }
    } catch (error) {
...
async function populateTeachingReportModal(data) {
    ...
    // Calculate recommended increase using database
...
async function downloadTeachingSummary(staffId) {
    try {
        ...
        const data = await response.json();
        await generateTeachingSummaryPDF(data);
...
async function generateTeachingSummaryPDF(data) {
    const { jsPDF } = window.jspdf;
   ...
   // Calculate average (same logic as your modal)
    ...
    async function calculateRecommendedIncreaseForPDF(totalPoints) {
        try {
            const response = await fetch(`${apiBaseUrl}/api/merit-pay/calculate?totalPoints=${totalPoints}`, {
                headers: {
                    "Authorization": `Bearer ${localStorage.getItem("accessToken")}`
            ...
            if (response.ok) {
                const data = await response.json();
                return data.inRange && data.payAmount > 0 ? `PHP ${parseFloat(data.payAmount).toFixed(2)}` : '';
            }
            return '';
        } catch (error) {
            console.error("Error calculating pay for PDF:", error);
            return '';
        }
    }
    
    const recommendedIncrease = await calculateRecommendedIncreaseForPDF(grandTotal);
