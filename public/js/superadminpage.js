...
async function loadPayRates() {
    try {
        const response = await fetch(`${apiBaseUrl}/api/superadmin/pay-rates`, {
            headers: {
                "Authorization": `Bearer ${localStorage.getItem("accessToken")}`
            }
        });
        ...
        if (response.ok) {
            payRatesData = await response.json();
            displayPayRatesEditor();
            displayPayRatesPreview();
        } else {
            document.getElementById("payRatesTableBody").innerHTML = 
                '<tr><td colspan="3" class="text-center text-danger">Failed to load pay rates</td></tr>';
        }
    } catch (error) {
        console.error("Error loading pay rates:", error);
        document.getElementById("payRatesTableBody").innerHTML = 
            '<tr><td colspan="3" class="text-center text-danger">Error loading pay rates</td></tr>';
    }
}

function displayPayRatesEditor() {
    const tbody = document.getElementById("payRatesTableBody");
        ...
        tbody.innerHTML = `
            <tr id="row_0">
                <td>
                    <div class="row g-2">
                        <div class="col">
                            <input type="number" class="form-control min-points" placeholder="Min" min="0" value=" ">
                        </div>
                        <div class="col-auto">
                            <span class="mt-2 d-block">-</span>
                        </div>
                        <div class="col">
                            <input type="number" class="form-control max-points" placeholder="Max" min="0" value=" ">
                        </div>
                    </div>
                </td>
                <td>
                    <div class="input-group">
                        ...
                        <input type="number" class="form-control pay-amount" step="0.01" min="0" value=" ">
                    </div>
                </td>
                <td class="text-center">
                    <button class="btn btn-sm btn-danger" onclick="removePayRateRow('row_0')" title="Remove">
                        <i class="fas fa-trash"></i>
                    </button>
                </td>
            </tr>
    ...
    // Sort by min points
    payRatesData.sort((a, b) => a.min_points - b.min_points);
    
    tbody.innerHTML = payRatesData.map((range, index) => `
        <tr id="row_${range.range_id || 'new_' + index}" data-range-id="${range.range_id || ''}">
            <td>
                <div class="row g-2">
                    <div class="col">
                        <input type="number" class="form-control min-points" placeholder="Min" min="0" 
                               value="${range.min_points}" required>
                    </div>
                    <div class="col-auto">
                        <span class="mt-2 d-block">-</span>
                    </div>
                    <div class="col">
                        <input type="number" class="form-control max-points" placeholder="Max" min="0" 
                               value="${range.max_points}" required>
                    </div>
                </div>
            </td>
            <td>
                <div class="input-group">
                    <span class="input-group-text">₱</span>
                    <input type="number" class="form-control pay-amount" step="0.01" min="0" 
                           value="${parseFloat(range.pay_amount).toFixed(2)}" required>
                </div>
            </td>
            <td class="text-center">
                <button class="btn btn-sm btn-danger" onclick="removePayRateRow('row_${range.range_id || 'new_' + index}')" title="Remove">
                    <i class="fas fa-trash"></i>
                </button>
            </td>
        </tr>
...
function displayPayRatesPreview() {
    const previewTbody = document.getElementById("payRatesPreview");
    
    if (payRatesData.length === 0) {
        previewTbody.innerHTML = '<tr><td colspan="2" class="text-center text-muted">No data to preview</td></tr>';
        return;
    }

    // Sort by min points
    const sortedRates = [...payRatesData].sort((a, b) => a.min_points - b.min_points);
    
    previewTbody.innerHTML = sortedRates.map(range => `
        <tr>
            <td><strong>${range.min_points} - ${range.max_points}</strong></td>
            <td><strong class="text-success">₱${parseFloat(range.pay_amount).toFixed(2)}</strong></td>
        </tr>
    `).join('');
}

function addNewPayRateRow() {
    const tbody = document.getElementById("payRatesTableBody");
    ...
    newRow.innerHTML = `
        ...
        <td class="text-center">
            <button class="btn btn-sm btn-danger" onclick="removePayRateRow('row_${newId}')" title="Remove">
                <i class="fas fa-trash"></i>
            </button>
        </td>
    `;
...
function removePayRateRow(rowId) {
    const row = document.getElementById(rowId);
    if (row) {
        const rangeId = row.getAttribute('data-range-id');
        
        // If it's an existing range (has ID), mark it for deletion
        if (rangeId) {
            row.classList.add('table-danger');
            row.style.opacity = '0.6';
            row.setAttribute('data-to-delete', 'true');
        } else {
...
async function saveAllPayRates() {
    const rows = document.querySelectorAll('#payRatesTableBody tr');
    const payRatesToSave = [];
    const payRatesToDelete = [];
    ...
    rows.forEach(row => {
        ...
        if (row.getAttribute('data-to-delete') === 'true') {
            const rangeId = row.getAttribute('data-range-id');
            if (rangeId) {
                payRatesToDelete.push(rangeId);
            }
            return;
        }
        
        const minInput = row.querySelector('.min-points');
        const maxInput = row.querySelector('.max-points');
        const payInput = row.querySelector('.pay-amount');
        
        const minPoints = parseInt(minInput.value);
        const maxPoints = parseInt(maxInput.value);
        const payAmount = parseFloat(payInput.value);
        const rangeId = row.getAttribute('data-range-id');
        
        // Validation
        if (!minPoints || !maxPoints || !payAmount) {
        ...
        if (minPoints >= maxPoints) {
            showToast("Max points must be greater than min points", true);
            hasError = true;
            return;
        }
        
        if (payAmount <= 0) {
            showToast("Pay amount must be greater than 0", true);
            hasError = true;
            return;
        }
        ...
        for (const range of ranges) {
            if ((minPoints <= range.max && maxPoints >= range.min)) {
                showToast(`Range ${minPoints}-${maxPoints} overlaps with ${range.min}-${range.max}`, true);
                hasError = true;
                return;
            }
        }
        
        ranges.push({ min: minPoints, max: maxPoints });
    ...
    // Confirm save
    if (!confirm(`Save ${payRatesToSave.length} pay rate range(s)?`)) {
        return;
    }
    
    try {
        const payload = {
            payRates: payRatesToSave,
            deleteRanges: payRatesToDelete
        };
        
        const response = await fetch(`${apiBaseUrl}/api/superadmin/pay-rates/batch`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${localStorage.getItem("accessToken")}`
        ...
        if (response.ok) {
            showToast("Pay rates saved successfully!");
            await loadPayRates(); // Reload to get updated data
        } else {
            const error = await response.json();
            alert(error.message || "Failed to save pay rates");
        }
    } catch (error) {
        console.error("Error saving pay rates:", error);
        alert("Error saving pay rates");
    }
}
