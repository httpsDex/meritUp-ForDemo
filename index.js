...
// ====================== PAY RATE MANAGEMENT API ======================

// Get all pay rates for superadmin
app.get("/api/superadmin/pay-rates", authenticate, requireRole(["Super_Admin"]), (req, res) => {
    const query = `
        SELECT range_id, min_points, max_points, pay_amount, is_active
        FROM merit_pay_ranges 
...
// Get single pay rate
app.get("/api/superadmin/pay-rates/:rangeId", authenticate, requireRole(["Super_Admin"]), (req, res) => {
    const rangeId = req.params.rangeId;
    
    const query = `
        SELECT * FROM merit_pay_ranges 
        WHERE range_id = ?
    `;
    
    connection.query(query, [rangeId], (err, results) => {
        if (err) {
        ...
        if (results.length === 0) {
            return res.status(404).json({ error: "Pay range not found" });
        }
...
// Add new pay rate
app.post("/api/superadmin/pay-rates", authenticate, requireRole(["Super_Admin"]), (req, res) => {
    const { minPoints, maxPoints, payAmount } = req.body;
    
    // Validate input
    if (minPoints === undefined || maxPoints === undefined || payAmount === undefined) {
        return res.status(400).json({ error: "Missing required fields" });
    }
    ...
    if (min >= max) {
        return res.status(400).json({ error: "Maximum points must be greater than minimum points" });
    }
    
    if (amount <= 0) {
        return res.status(400).json({ error: "Pay amount must be greater than 0" });
    }
    ...
    const checkQuery = `
        ...
        AND (
            (min_points <= ? AND max_points >= ?) OR
            (min_points <= ? AND max_points >= ?) OR
            (min_points >= ? AND max_points <= ?)
        )
    `;
    
    connection.query(checkQuery, [min, min, max, max, min, max], (err, results) => {
        if (err) {
            console.error("Error checking overlapping ranges:", err);
            return res.status(500).json({ error: "Server error", details: err.message });
        }
        
        if (results.length > 0) {
            return res.status(400).json({ 
                error: "This range overlaps with existing active range(s)",
                overlappingRanges: results 
            });
        }
        
        // Insert new range
        const insertQuery = `
            INSERT INTO merit_pay_ranges 
            (min_points, max_points, pay_amount, is_active) 
            VALUES (?, ?, ?, TRUE)
        `;
        
        connection.query(insertQuery, [min, max, amount], (err, result) => {
            if (err) {
            ...
            res.status(201).json({ 
                message: "Pay range added successfully", 
...
// Update pay rate
app.put("/api/superadmin/pay-rates/:rangeId", authenticate, requireRole(["Super_Admin"]), (req, res) => {
    const rangeId = req.params.rangeId;
    const { minPoints, maxPoints, payAmount } = req.body;
    ...
    const checkExistenceQuery = "SELECT * FROM merit_pay_ranges WHERE range_id = ?";
    connection.query(checkExistenceQuery, [rangeId], (err, results) => {
        if (err) {
            console.error("Error checking range existence:", err);
            return res.status(500).json({ error: "Server error", details: err.message });
        }
        ...
        connection.query(checkOverlapQuery, [rangeId, min, min, max, max, min, max], (err, overlapResults) => {
            if (err) {
                console.error("Error checking overlapping ranges:", err);
                return res.status(500).json({ error: "Server error", details: err.message });
            }
            ...
            if (overlapResults.length > 0) {
                return res.status(400).json({ 
                    error: "This range overlaps with existing active range(s)",
                    overlappingRanges: overlapResults 
                });
            }
            
            // Update range
            const updateQuery = `
                UPDATE merit_pay_ranges 
                SET min_points = ?, max_points = ?, pay_amount = ?, updated_at = CURRENT_TIMESTAMP 
                WHERE range_id = ?
            `;
            
            connection.query(updateQuery, [min, max, amount, rangeId], (err, result) => {
                if (err) {
                ...
                res.json({ 
                    message: "Pay range updated successfully",
...
// Delete pay rate
app.delete("/api/superadmin/pay-rates/:rangeId", authenticate, requireRole(["Super_Admin"]), (req, res) => {
    const rangeId = req.params.rangeId;
    
    const query = "DELETE FROM merit_pay_ranges WHERE range_id = ?";
    
    connection.query(query, [rangeId], (err, result) => {
        if (err) {
        ...
        res.json({ 
            message: "Pay range deleted successfully",
            affectedRows: result.affectedRows 
        });
    });
});

// Batch save pay rates (for table-based editor)
app.post("/api/superadmin/pay-rates/batch", authenticate, requireRole(["Super_Admin"]), (req, res) => {
    const { payRates, deleteRanges } = req.body;
    
    console.log("Batch save request:", { payRates, deleteRanges });
    
    // Validate input
    if (!payRates || !Array.isArray(payRates)) {
        return res.status(400).json({ error: "Invalid payRates data" });
    }
    
    // Validate each pay rate
    for (const rate of payRates) {
        if (!rate.minPoints || !rate.maxPoints || !rate.payAmount) {
            return res.status(400).json({ error: "Missing fields in pay rate data" });
        }
        
        const min = parseInt(rate.minPoints);
        const max = parseInt(rate.maxPoints);
        const amount = parseFloat(rate.payAmount);
        ...
        if (min >= max) {
            return res.status(400).json({ error: `Maximum points must be greater than minimum points (${min}-${max})` });
        }
    ...
    // Check for overlapping ranges among new/updated rates
    for (let i = 0; i < payRates.length; i++) {
        for (let j = i + 1; j < payRates.length; j++) {
            const a = payRates[i];
            const b = payRates[j];
            
            if ((a.minPoints <= b.maxPoints && a.maxPoints >= b.minPoints)) {
                return res.status(400).json({ 
                    error: `Ranges overlap: ${a.minPoints}-${a.maxPoints} and ${b.minPoints}-${b.maxPoints}` 
                });
            }
        }
    }
    ...
    connection.beginTransaction((err) => {
        ...
        if (deleteRanges && deleteRanges.length > 0) {
            deleteRanges.forEach(rangeId => {
                promises.push(new Promise((resolve, reject) => {
                    connection.query("DELETE FROM merit_pay_ranges WHERE range_id = ?", [rangeId], (err, result) => {
                        if (err) {
                            console.error("Delete error for rangeId", rangeId, ":", err);
                            reject(err);
                        } else {
                            console.log("Deleted range", rangeId, "affected rows:", result.affectedRows);
                            resolve(result);
                        }
                    });
                }));
            ...
            promises.push(new Promise((resolve, reject) => {
                if (rate.rangeId) {
                    ...
                    connection.query(query, [
                        ...
                        if (err) {
                            console.error("Update error for rangeId", rate.rangeId, ":", err);
                            reject(err);
                        } else {
                            console.log("Updated range", rate.rangeId, "affected rows:", result.affectedRows);
                            resolve(result);
                        }
                    });
                } else {
                    ...
                    connection.query(query, [
                        ...
                        if (err) {
                            ...
                            console.log("Inserted new range, ID:", result.insertId);
                            resolve(result);
                        }
                    });
                }
        ...
        Promise.all(promises)
            .then(() => {
                ...
                connection.commit((err) => {
                    if (err) {
                        ...
                        res.json({ 
                            message: "Pay rates saved successfully",
                            savedCount: payRates.length,
                            deletedCount: deleteRanges ? deleteRanges.length : 0
                        });
                    }
                });
            })
            .catch((err) => {
                ...
                connection.rollback(() => {
                    res.status(500).json({ error: "Failed to save pay rates", details: err.message });
                });
            });
    });
});

// Get pay amount for given points (for frontend calculations)
app.get("/api/merit-pay/calculate", authenticate, (req, res) => {
    const { totalPoints } = req.query;
    
    if (!totalPoints) {
        return res.status(400).json({ error: "Total points required" });
    }
    ...
    connection.query(query, [points], (err, results) => {
        if (err) {
            console.error("Error calculating merit pay:", err);
            return res.status(500).json({ error: "Server error", details: err.message });
        }
        ...
        if (results.length === 0) {
            return res.json({ 
                payAmount: 0,
                message: "No pay range found for these points",
                inRange: false 
            });
        }
        
        res.json({ 
            payAmount: results[0].pay_amount,
            range: results[0],
            inRange: true 
        });
    });
...
// Get active pay rates for public use (no auth required for frontend display)
app.get("/api/public/pay-rates", (req, res) => {
    const query = `
        SELECT min_points, max_points, pay_amount 
        FROM merit_pay_ranges 
        WHERE is_active = TRUE 
        ORDER BY min_points ASC
    `;
    ...
    connection.query(query, (err, results) => {
        if (err) {
            console.error("Error fetching public pay rates:", err);
            return res.status(500).json({ error: "Server error", details: err.message });
        }
        res.json(results);
    });
